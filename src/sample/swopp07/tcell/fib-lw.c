...
do_fib_task (char *esp, struct thread_data *_thr, struct fib *pthis)
{
  ...
  *((char **) esp) = 0;
  ...
  new_esp = esp;
  while ((tmp = fib (new_esp, &efp->_bk, _thr, (*pthis).n)) == (int) 0 - 1
  ...
  if (n <= 2)
	  ...
	  }
	{
	  ...
	  if ((*_thr).req)
	    {
	      new_esp = esp;
	      while (handle_req (new_esp, &efp->_bk2, _thr),
		     (efp->tmp_esp = *((char **) esp)) != 0)
		{
		  efp->_bk0 = _bk0;
		  efp->spawned = spawned;
	    ...
	    }
	  {
	    new_esp = esp;
	    while ((s1 =
		    fib (new_esp, &efp->_bk2, _thr, n - 1)) == (int) 0 - 1
		   && (efp->tmp_esp = *((char **) esp)) != 0)
	      {
		efp->_bk0 = _bk0;
		efp->spawned = spawned;
	  ...
	  if (spawned)
	  ...
	  else
	    {
	      new_esp = esp;
	      while ((s2 =
		      fib (new_esp, &efp->_bk2, _thr, n - 2)) == (int) 0 - 1
		     && (efp->tmp_esp = *((char **) esp)) != 0)
		{
		  efp->_bk0 = _bk0;
		  efp->spawned = spawned;
