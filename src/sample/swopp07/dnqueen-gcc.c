...
int nqueens(proc_env pr, CLT int (*bkf)(req_buf_t *), int n, int k);

void tf_backup_and_nqueens(proc_env pr, task_buf_t *task_buf_p){
  CLT int bk(req_buf_t *req_buf_p){ return 0; }
  int a_bak[20];
  int lb_bak[40];
  int rb_bak[40];
  int jj;
...
int 
nqueens(proc_env pr, CLT int (*bkf)(req_buf_t *), int n, int k)
{
  if (n == k)
    return 1;
  else
    {
      ...
      CLT int bk(req_buf_t *req_buf_p){
      ...
      while(si < ei)
	{
	  ...
	  POLL(pr->req_port, bk);
	  {
	    ...
	    int ai = pr->a[i];
	    if (
		(! (pr->lb[n - 1 - ai + k] || pr->rb[ai + k]))){
	      int ak = pr->a[k];
	      pr->a[i] = ak; pr->a[k] = ai;
	      pr->lb[n - 1 - ai + k] = 1; pr->rb[ai + k] = 1;
	      {
		CLT int bk2(req_buf_t *req_buf_p){
		  int r;
		  *(&ai) = pr->a[k]; *(&ak) = pr->a[i];
		  pr->a[i] = ai; pr->a[k] = ak;
		  pr->lb[n - 1 - ai + k] = 0; pr->rb[ai + k] = 0;
