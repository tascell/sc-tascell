    ...
    if (n1 > n2) {
...

        {
                ...
                send_divisible();
            {
                ...
                int do_two_bk3(void) {
                    ...
                    _bk();
                    if ((*_thr).treq_top) {
                        put_task(&(*pthis), TASK_RD, n1_2, n2, n3, b1 + n1_1,
                                 b2 + n1_1 * nx, 0, nx, anx);
                        spawned3 = 1;
                        make_and_send_task(_thr, 0, pthis);
                        return 1;
                ...
                {
                    decomp_rd(do_two_bk3, _thr, n1_1, n2, n3, b1, b2, a, nx, anx);
                }
            }
            if (spawned3) {
                wait_rslt(_thr);
...

        {
                ...
                send_divisible();
            {
                ...
                if ((*_thr).req)
                    handle_req(do_two_bk4, _thr);
                {
                    decomp_rd(do_two_bk4, _thr, n1, n2_1, n3, b1, b2, a, nx, anx);
                }
            }
            if (spawned4) {
                wait_rslt(_thr);
                ...
                decomp_rd(_bk, _thr, n1, n2_2, n3, b1 + n2_1 * nx, b2, a + n2_1 * anx,
                          nx, anx);
            }
        }
    }
...
void input_mat(double *a, int n1, int n2, int nx)
{
    ...
    read_to_eol();
    scanf(" %*d %*d %d", &e);
    fprintf (stderr, "input_mat(e)=%d\n",e); // debug print
    read_to_eol();
    {
        size_t total_sz = 0; // debug print
        i = 0;
        for (; i < n1; i++) {
            size_t sz = 0;

            while (sz < n2) {
                total_sz += sizeof(double)*(n2-sz); // debug print
                sz += fread(a + (i * nx + sz), sizeof(double), n2 - sz, stdin);
            }
        }
        fprintf (stderr, "Read %d bytes.\n",total_sz); // debug print
    }
    scanf(" )");
    if ((int)(x.c)[0] != e) {
        i = 0;
...
void output_mat_l(double *a, int n1, int nx)
{
    ...
    x.i = 1;
    printf(" (\n %d %d %d\n", sizeof(double) * n1 * (n1 + 1) / 2, sizeof(double),
           (int)(x.c)[0]);
    {
        i = 0;
        for (; i < n1; i++) {
...
void input_mat_u(double *a, int n1, int nx)
{
    ...
    read_to_eol();
    {
        ...
        for (; i < n1; i++) {
            ...
            while (sz < n1 - i - 1) {
                sz +=
                    fread(a + (i * nx + i + 1 + sz), sizeof(double), n1 - i - 1 - sz,
                          stdin);
            }
        }
    }
...
void output_mat_u(double *a, int n1, int nx)
{
           ...
           (int)(x.c)[0]);
    {
        ...
        for (; i < n1; i++) {
            ...
            while (sz < n1 - i - 1) {
                sz +=
                    fwrite(a + (i * nx + i + 1 + sz), sizeof(double), n1 - i - 1 - sz,
                           stdout);
            }
        }
    }
...
struct task_lu *recv_task_lu_task(struct thread_data *_thr)
{
    ...
    case TASK_RD:
        ...
        n3 = recv_int();
        nx = n1 + n3;
        fprintf (stderr, "recv(n1,n2,n3,nx)=(%d,%d,%d,%d)\n",n1,n2,n3,nx); // debug print
