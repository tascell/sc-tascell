...
int		search     (int (*_bk) closure(void), struct thread_data *_thr, int k, int j0, int j1, int j2, struct pentomino *tsk);
...

  {
	...
	for (; i < pos[ap + 1]; i++) {
	  ...
	  }
	  {
		...
		if (kk == 70) {
		  s += 1;
		} else if ((kk + 7 >= 70 || ((*tsk).b)[kk + 7] != 0) && (((*tsk).b)[kk + 1] != 0 || (kk + 8 >= 70 || ((*tsk).b)[kk + 8] != 0) && ((*tsk).b)[kk + 2] != 0)) {
		  ;
		} else {
		  s += search(_bk, _thr, kk, j0 + 1, j0 + 1, 12, tsk);
		}
	      }
	      {
		ap = ((*tsk).a)[j0];
