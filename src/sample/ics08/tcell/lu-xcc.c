...
void		decomp_lu (int (*_bk) lightweight(void), struct thread_data *_thr, int n, double *a, int nx, int anx);

void		decomp_r  (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, double *b, double *a, int nx, int an);

void		decomp_d  (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, double *b, double *a, int nx, int an);

void		decomp_rd (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, int n3, double *b1, double *b2, double *a, int nx, int anx);
...
void		decomp_lu_0(int n, double *a, int nx, int anx)
{
  ...
  }
  {
	...
	w = 1.0 / a[i * anx + i];
	{
	  j = i + 1;
	  for (; j < n; j++) {
	    a[i * anx + j] = w * (a[i * anx + j] - aik * a[(i - 1) * anx + j]);
	  }
	}
      }
    }
...
void		decomp_lu (int (*_bk) lightweight(void), struct thread_data *_thr, int n, double *a, int nx, int anx){
  ...
  if (n <= 4) {
	...
	int _bk		lightweight(void){
	  ...
	  _bk0();
	  if ((*_thr).treq_top) {
	    put_task(&(*pthis), TASK_D, n1, n2, 0, a, 0, a + n1 * nx, nx, anx);
	    spawned = 1;
    ...
    decomp_rd(_bk, _thr, n1, n2, n2, a + n1 * nx, a + n1, a + (n1 * nx + n1), nx, anx);
...
void		decomp_r  (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, double *b, double *a, int nx, int anx){
  if (n1 <= 4) {
    ...
    decomp_rd(_bk, _thr, n1_1, n1_2, n2, b + n1_1 * nx, a, a + n1_1 * nx, nx, anx);
    decomp_r(_bk, _thr, n1_2, n2, b + (n1_1 * nx + n1_1), a + n1_1 * nx, nx, anx);
  }
}
...
void		decomp_d  (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, double *b, double *a, int nx, int anx){
  if (n2 <= 4) {
	...
	int _bk		lightweight(void){
	  ...
	  _bk0();
	  if ((*_thr).treq_top) {
	    put_task(&(*pthis), TASK_D, n1, n2_2, 0, b, 0, a + n2_1 * nx, nx, anx);
	    spawned2 = 1;
	    make_and_send_task(_thr, 0, pthis);
	    return 1;
	  }
...
void		decomp_rd (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, int n3, double *b1, double *b2, double *a, int nx, int anx){
  ...
  if (n1 > n2) {
	...
	int _bk		lightweight(void){
	  ...
	  _bk0();
	  if ((*_thr).treq_top) {
	    put_task(&(*pthis), TASK_RD, n1_2, n2, n3, b1 + n1_1, b2 + n1_1 * nx, 0, nx, anx);
	    spawned3 = 1;
	    make_and_send_task(_thr, 0, pthis);
	    return 1;
	  }
	...
	if (spawned3) {
	  ...
	  decomp_rd(_bk, _thr, n1_2, n2, n3, b1 + n1_1, b2 + n1_1 * nx, a, nx, anx);
	}
      }
    }
  } else {
	...
	int _bk		lightweight(void){
	  ...
	  _bk0();
	  if ((*_thr).treq_top) {
	    put_task(&(*pthis), TASK_RD, n1, n2_2, n3, b1 + n2_1 * nx, b2, a + n2_1 * anx, nx, anx);
	    spawned4 = 1;
	    make_and_send_task(_thr, 0, pthis);
	    return 1;
	  }
	...
	if (spawned4) {
	  ...
	  decomp_rd(_bk, _thr, n1, n2_2, n3, b1 + n2_1 * nx, b2, a + n2_1 * anx, nx, anx);
	}
      }
    }
  }
...
void		decomp_lu_1(int (*_bk) lightweight(void), struct thread_data *_thr, int n, double *a, int nx, int anx){
  int		  i;

  {
	...
	n1 = n - i;
      ...
      decomp_rd(_bk, _thr, i, n1, n - i, a + i * nx, a + i, a + (i * nx + i), nx, anx);
...
void		proc_cmd  (int (*_bk) lightweight(void), struct thread_data *_thr, int n1, int n2, int n3, double *ansp){
  struct timeval  tp[2];
  int		  n = n1;
  int		  al = n2;
  int		  d = n3;
