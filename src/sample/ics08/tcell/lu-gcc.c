...
void
decomp_lu (int (*_bk) (void), struct thread_data *_thr, int n, double *a,
           int nx, int anx)
{
  if (n <= 4)
    {
...
void
decomp_r (int (*_bk) (void), struct thread_data *_thr, int n1, int n2,
...
void
decomp_d (int (*_bk) (void), struct thread_data *_thr, int n1, int n2,
          double *b, double *a, int nx, int anx)
{
  if (n2 <= 4)
    {
    ...
    int n2_2 = n2 - n2_1;
    {
        ...
        send_divisible ();
      {
        ...
        int do_two_bk2 (void)
        {
          ...
          if ((*_thr).treq_top)
            {
              put_task (&(*pthis), TASK_D, n1, n2_2, 0, b, 0, a + n2_1 * nx,
                        nx, anx);
              spawned2 = 1;
              make_and_send_task (_thr, 0, pthis);
              return 1;
...
void
decomp_rd (int (*_bk) (void), struct thread_data *_thr, int n1, int n2,
           int n3, double *b1, double *b2, double *a, int nx, int anx)
{
  if (n1 <= 4 && n2 <= 4)
    {
      ...
      int n2_2 = n2 - n2_1;
      {
          ...
          send_divisible ();
        {
          ...
          int do_two_bk3 (void)
          {
            ...
            if ((*_thr).treq_top)
              {
                put_task (&(*pthis), TASK_RD, n1, n2_2, n3, b1 + n2_1 * nx,
                          b2, a + n2_1 * anx, nx, anx);
                spawned3 = 1;
                make_and_send_task (_thr, 0, pthis);
                return 1;
              }
...
void
decomp_lu_1 (int (*_bk) (void), struct thread_data *_thr, int n, double *a,
             int nx, int anx)
{
  int i;
  {
    ...
    for (; i < n; i += 16)
      {
        ...
        decomp_rd (_bk, _thr, i, n1, n - i, a + i * nx, a + i,
                   a + (i * nx + i), nx, anx);
...
void
proc_cmd (int (*_bk) (void), struct thread_data *_thr, int n1, int n2, int n3,
          double *ansp)
{
  struct timeval tp[2];
  int n = n1;
