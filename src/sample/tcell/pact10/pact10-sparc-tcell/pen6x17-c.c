...
int search (int k, int j0, int j1, int j2, int maxp, int maxk,
...
void do_pentomino_task (struct thread_data *_thr, struct pentomino *pthis)
{
  ...
  (*pthis).r = search (k, i0, i1, i2, maxp, maxk, &(*pthis));
  {
  }
}
                              ...
                              struct start_pentomino *pthis)
{
  ...
  pen.maxp = maxp;
  gettimeofday (tp, 0);
  (*pthis).r = search (0, 0, 0, maxp, maxp, pen.maxk, &pen);
  gettimeofday (tp + 1, 0);
                ...
                {
                  {
                    ...
                    if (kk == maxk)
                      ...
                      s += search (kk, j0 + 1, j0 + 1, maxp, maxp, maxk, tsk);
                  }
                  {
                    ap = ((*tsk).a)[j0];
                    ((*tsk).a)[j0] = ((*tsk).a)[p];
