...
LGOTO:;
  ...
  esp =
    (char *) ((Align_t *) esp +
              (sizeof (struct do_two_bk_in_decomp_lu_frame) + sizeof (Align_t) +
               -1) / sizeof (Align_t));
  *((char **) esp) = 0;
  if (xfp->spawned)
    {
    ...
    }
    {
      ...
      spawned = 0;
      {
        ...
        else;
        {
          new_esp = esp;
          while (decomp_r
                 (new_esp, &efp->do_two_bk, _thr, n1, n2, a, a + n1, nx, anx),
                 __builtin_expect ((efp->tmp_esp = *((char **) esp)) != 0, 0))
            {
              efp->spawned = spawned;
              efp->pthis = pthis;
      ...
      if (spawned)
        ...
        {
          new_esp = esp;
          while (decomp_d (new_esp, _bk, _thr, n1, n2, a, a + n1 * nx, nx, anx),
                 __builtin_expect ((efp->tmp_esp = *((char **) esp)) != 0, 0))
            {
              efp->spawned = spawned;
              efp->pthis = pthis;
...
LGOTO:;
  ...
  esp =
    (char *) ((Align_t *) esp +
              (sizeof (struct do_two_bk2_in_decomp_d_frame) + sizeof (Align_t) +
               -1) / sizeof (Align_t));
  *((char **) esp) = 0;
  if (xfp->spawned2)
    {
        ...
        else
          {
            ...
            while (decomp_rd
                   (new_esp, _bk, _thr, n1, n2_2, n3, b1 + n2_1 * nx, b2,
                    a + n2_1 * anx, nx, anx), __builtin_expect ((efp->tmp_esp =
                                                                 *((char **)
                                                                   esp)) != 0,
                                                                0))
              {
...
void decomp_lu_1 (char *esp, closure_t * _bk, struct thread_data *_thr, int n,
                  double *a, int nx, int anx)
{
  int i;
  int n1;
...
void start_lu (char *esp, closure_t * _bk, struct thread_data *_thr, int n1,
...
LGOTO:;
  ...
  esp =
    (char *) ((Align_t *) esp +
              (sizeof (struct _bk_in_do_task_lu_task_frame) + sizeof (Align_t) +
               -1) / sizeof (Align_t));
  *((char **) esp) = 0;
  {
    *((int *) efp) = 0;
...
void do_task_lu_task (char *esp, struct thread_data *_thr,
                      struct task_lu *pthis)
{
  int ifexp_result4;
  int n1;
...
struct task_lu *recv_task_lu_task ()
{
  ...
  int nx;
  static double ans;
  pthis = malloc (sizeof (struct task_lu));
  tt = recv_int ();
  switch (tt)
    {
    case TASK_CMD:
