...
int N_Iter = 0;
int V_List_Remain[1000];
int Visited_Vertices[1000];

int count_visited_vertices (void)
{
  int v;
  int n = 0;
...
void print_statistics (void)
{
  ...
  fprintf (stderr, "Iterations: %d\n", N_Iter);
  {
    ...
    for (; i < N_Iter; i++)
      {
        fprintf (stderr, "iter(%3d): list-length=%9d         visited=%9d\n",
                 i + 1, V_List_Remain[i], Visited_Vertices[i]);
      }
  }
}
...
void search1_p (int (*_bk) closure (void), struct thread_data *_thr, int v,
                int b, int k0, int k1);

void search_p_b (int (*_bk) closure (void), struct thread_data *_thr,
                 struct v_list_body *my_v_list, int i0, int i1);

void search_p_b_array (int (*_bk) closure (void), struct thread_data *_thr,
                       struct v_list_body **my_v_array, int i0, int i1);

void search_p_c (int (*_bk) closure (void), struct thread_data *_thr,
...
void search_p (int (*_bk) closure (void), struct thread_data *_thr, int v)
{
  ...
  while ((*((struct worker_data *) (*_thr).wdptr)).V_List.length > 1
         || (*(*((struct worker_data *) (*_thr).wdptr)).V_List.first).i < 600)
    {
      V_List_Remain[N_Iter] =
        (*((struct worker_data *) (*_thr).wdptr)).V_List.length;
      Visited_Vertices[N_Iter] = count_visited_vertices ();
      N_Iter++;
...
void search_p_array (int (*_bk) closure (void), struct thread_data *_thr, int v)
{
