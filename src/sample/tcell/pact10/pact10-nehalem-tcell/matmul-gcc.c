...
enum command { TASK, RSLT, TREQ, NONE, BACK, RACK, DREQ, DATA, STAT, VERB, EXIT, WRNG};
...
struct tmatmul{
 ...
 void matmul_1 (int (*_bk)( void  ), struct thread_data *_thr, int , int , int , int , int , double *, double *, double *, int , struct tmatmul *);

 void matmul_2 (int , int , int , int , int , int , double *, double *, double *, int );

 void flush_subtask_queue (int (*_bk)( void  ), struct thread_data *_thr, int , double *, double *, double *, int , struct tmatmul *);
...
if(option.verbose >= 1 )
fprintf(stderr, "sub-matmul(%d %d %d %d)\n", (*pthis).i0, (*pthis).i1, (*pthis).j0, (*pthis).j1);
else;matmul_1(_bk, _thr, (*pthis).i0, (*pthis).i1, (*pthis).j0, (*pthis).j1, (*pthis).n, A, B, C, (*pthis).n, &(*pthis));
...
gettimeofday(tp, 0);
matmul_1(_bk, _thr, mtask.i0, mtask.i1, mtask.j0, mtask.j1, mtask.n, A, B, C, mtask.n, &mtask);
...
{
 double *c2;
c2 = (double *)malloc(sizeof(double )*(*pthis).n*(*pthis).n);
matmul_2(0, (*pthis).n, 0, (*pthis).n, 0, (*pthis).n, A, B, c2, (*pthis).n);
fprintf(stderr, "diff: %lf\n", diffmat((*pthis).n, C, c2, (*pthis).n));
free(c2);
}
else;}

 void matmul_2 (int i0, int i1, int j0, int j1, int k0, int k1, double *a, double *b, double *c, int nx)
{
 int i;
 int j;
 int k;
...
return ;
 ...
 void matmul_m (int i0, int i1, int j0, int j1, int k0, int k1, double *a, double *b, double *c, int nx)
{
if(option.verbose >= 2 )
...
matmul_m(i0, i1, half, j1, k0, k1, a, b, c, nx);
 ...
 void matmul_s (int (*_bk)( void  ), struct thread_data *_thr, int i0, int i1, int j0, int j1, int n, double *a, double *b, double *c, int nx, struct tmatmul *tsk)
{
...
head = 0;
 ...
 void flush_subtask_queue (int (*_bk)( void  ), struct thread_data *_thr, int n, double *a, double *b, double *c, int nx, struct tmatmul *tsk)
{
 int m;
 int head = (*tsk).head;
 struct mm_subtask *sq = (*tsk).queue;
...
head = 0;
 ...
 void matmul_1 (int (*_bk)( void  ), struct thread_data *_thr, int i0, int i1, int j0, int j1, int n, double *a, double *b, double *c, int nx, struct tmatmul *tsk)
{
if(option.verbose >= 2 )
 ...
 void send_tmatmul_rslt (struct tmatmul *pthis)
{
 ...
 int jdiff;
if(option.verbose >= 2 )
fprintf(stderr, "rslt-sender-start\n");
else;idiff = (*pthis).i1-(*pthis).i0;
...
if(option.verbose >= 2 )
fprintf(stderr, "rslt-sender-end\n");
else;free(pthis);
}
 ...
 void recv_tmatmul_rslt (struct tmatmul *pthis)
{
 ...
 int swp;
if(option.verbose >= 2 )
fprintf(stderr, "rslt-receiver-start\n");
else;idiff = (*pthis).i1-(*pthis).i0;
jdiff = (*pthis).j1-(*pthis).j0;
swp = recv_binary_header(sizeof(double ), idiff*jdiff);
{
i = (*pthis).i0;
for(  ; i < (*pthis).i1 ; i++ )
{
...
fprintf(stderr, "rslt-receiver-end\n");
else;}
 void (*task_doers[256])( struct thread_data *, void * ) = { (void (*)( struct thread_data *, void * ))do_tmatmul_task, (void (*)( struct thread_data *, void * ))do_matmul_start_task };
 void (*task_senders[256])( void * ) = { (void (*)( void * ))send_tmatmul_task, (void (*)( void * ))send_matmul_start_task };
 void *(*task_receivers[256])(  ) = { (void *(*)(  ))recv_tmatmul_task, (void *(*)(  ))recv_matmul_start_task };
 void (*rslt_senders[256])( void * ) = { (void (*)( void * ))send_tmatmul_rslt, (void (*)( void * ))send_matmul_start_rslt };
 void (*rslt_receivers[256])( void * ) = { (void (*)( void * ))recv_tmatmul_rslt, (void (*)( void * ))recv_matmul_start_rslt };
