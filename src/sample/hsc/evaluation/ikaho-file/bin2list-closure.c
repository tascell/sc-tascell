...
lgoto:
  ...
  *((char **)esp) = 0;
  ...
  case 0:
    while (gc_breadth_first(esp, efp->scan), (efp->tmp_esp = *((char **)esp)) != 0) {
...
size_t          _KVpair_node[] = {};
descriptor      _KVpair_d = {sizeof(struct _KVpair), sizeof(align_t) * ((sizeof(struct _KVpair) + sizeof(align_t) - 1) / sizeof(align_t)), sizeof(_KVpair_node) / sizeof(size_t), _KVpair_node};
...
size_t          _Alist_node[] = {(size_t) & (*((struct _Alist *) 0)).kv, (size_t) & (*((struct _Alist *) 0)).cdr};
descriptor      _Alist_d = {sizeof(struct _Alist), sizeof(align_t) * ((sizeof(struct _Alist) + sizeof(align_t) - 1) / sizeof(align_t)), sizeof(_Alist_node) / sizeof(size_t), _Alist_node};
...
lgoto:
  ...
  *((char **)esp) = 0;
  ...
  tmp_arg = &_Bintree_d;
  while ((efp->tmp3 = getmem(esp, &efp->scan1, tmp_arg)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (Bintree *) 0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  }
  efp->tmp_object = (struct _Bintree *) efp->tmp3;
  (*efp->tmp_object).key = efp->k;
  (*efp->tmp_object).val = efp->v;
  (*efp->tmp_object).left = 0;
  (*efp->tmp_object).right = 0;
  efp->p = efp->tmp_object;
  return efp->p;
}
  ...
  *((char **)esp) = 0;
  ...
  do {
    if ((*efp->x).key == efp->k) {
      ...
      efp->y = (*efp->x).left;
      if (!efp->y) {
	while ((efp->tmp4 = newBintree(esp, &efp->scan1, efp->k, efp->v)) == (struct _Bintree *) 0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
	  if ((char *)efp->tmp_esp < (char *)efp)
	    return;
	  efp->tmp_esp = 0;
	  goto lgoto;
	...
	return;
      ...
      efp->y = (*efp->x).right;
      if (!efp->y) {
	while ((efp->tmp5 = newBintree(esp, &efp->scan1, efp->k, efp->v)) == (struct _Bintree *) 0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
	  if ((char *)efp->tmp_esp < (char *)efp)
	    return;
	  efp->tmp_esp = 0;
	  goto lgoto;
...
struct bin2list_frame {
  ...
  KVpair         *kv;
  Alist          *a;
  struct _KVpair *tmp_object2;
  struct _Alist  *tmp_object3;
  Alist          *rest;
  Bintree        *x;
  sht             scan0;
  struct _Bintree *tmp_arg5;
  struct unnamed1 *tmp_arg4;
  struct unnamed1 *tmp_arg3;
  struct _Bintree *tmp_arg2;
  closure_t       scan1;
};
...
char           *
scan1_in_bin2list(char *esp, void *xfp0)
{
...
lgoto:
  ...
  esp = (char *)((Align_t *) esp + (sizeof(struct scan1_in_bin2list_frame) + sizeof(Align_t) + -1) / sizeof(Align_t));
  *((char **)esp) = 0;
  efp->xfp = xfp;
  xfp->x = move(xfp->x);
...
Alist          *
bin2list(char *esp, sht scan0, Bintree * x, Alist * rest)
{
...
lgoto:
  ...
  esp = (char *)((Align_t *) esp + (sizeof(struct bin2list_frame) + sizeof(Align_t) + -1) / sizeof(Align_t));
  *((char **)esp) = 0;
  efp->scan0 = scan0;
  efp->x = x;
  efp->rest = rest;
  ...
  efp->kv = 0;
  if ((*efp->x).right) {
    tmp_arg2 = (*efp->x).right;
    while ((efp->rest = bin2list(esp, &efp->scan1, tmp_arg2, efp->rest)) == (struct _Alist *) 0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
      if ((char *)efp->tmp_esp < (char *)efp)
	return (Alist *) 0 - 1;
      efp->tmp_esp = 0;
      goto lgoto;
  ...
  tmp_arg3 = &_KVpair_d;
  while ((efp->tmp6 = getmem(esp, &efp->scan1, tmp_arg3)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (Alist *) 0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  }
  efp->tmp_object2 = (struct _KVpair *) efp->tmp6;
  (*efp->tmp_object2).key = (*efp->x).key;
  (*efp->tmp_object2).val = (*efp->x).val;
  efp->kv = efp->tmp_object2;
  tmp_arg4 = &_Alist_d;
  while ((efp->tmp7 = getmem(esp, &efp->scan1, tmp_arg4)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (Alist *) 0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  }
  efp->tmp_object3 = (struct _Alist *) efp->tmp7;
  (*efp->tmp_object3).kv = efp->kv;
  (*efp->tmp_object3).cdr = efp->rest;
  efp->a = efp->tmp_object3;
  efp->rest = efp->a;
  if ((*efp->x).left) {
    tmp_arg5 = (*efp->x).left;
    while ((efp->rest = bin2list(esp, &efp->scan1, tmp_arg5, efp->rest)) == (struct _Alist *) 0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
      if ((char *)efp->tmp_esp < (char *)efp)
	return (Alist *) 0 - 1;
      efp->tmp_esp = 0;
      goto lgoto;
...
lgoto:
  ...
  *((char **)esp) = 0;
  ...
  (efp->seed)[2] = 5;
  {
	...
	while (insert(esp, &efp->scan1, efp->this, efp->k, efp->k), (efp->tmp_esp = *((char **)esp)) != 0) {
	  if ((char *)efp->tmp_esp < (char *)efp)
	    return;
	  efp->tmp_esp = 0;
	  goto lgoto;
  ...
  *((char **)esp) = 0;
  ...
  (efp->seed)[2] = 10;
  {
    ...
    if (efp->i < efp->n)
      do {
	efp->k = nrand48(efp->seed);
	while (search(esp, &efp->scan1, efp->this, efp->k, 0), (efp->tmp_esp = *((char **)esp)) != 0) {
	  if ((char *)efp->tmp_esp < (char *)efp)
	    return;
	  efp->tmp_esp = 0;
	  goto lgoto;
...
struct main_frame {
  ...
  struct timeval  tp2;
  struct timeval  tp1;
  struct _Bintree *tmp_object4;
  char          **argv;
  int             argc;
  struct unnamed1 *tmp_arg6;
  closure_t       scan1;
};
...
lgoto:
  ...
  *((char **)esp) = 0;
  ...
  efp->searchskip = 0;
  if (efp->argc > 3) {
    efp->tmp8 = atoi((efp->argv)[3]);
    efp->ifexp_result = efp->tmp8;
  } else {
    efp->ifexp_result = 0;
  }
  efp->gctp = efp->ifexp_result;
  if (efp->argc > 4) {
    efp->tmp9 = atoi((efp->argv)[4]);
    efp->ifexp_result3 = efp->tmp9;
  } else {
    efp->ifexp_result3 = 0;
  }
  if (efp->argc > 5) {
    efp->tmp10 = atoi((efp->argv)[5]);
    efp->ifexp_result4 = efp->tmp10;
  } else {
    efp->ifexp_result4 = 0;
  }
  if (efp->argc > 6) {
    efp->tmp11 = atoi((efp->argv)[6]);
    efp->ifexp_result5 = efp->tmp11;
  } else {
    efp->ifexp_result5 = 0;
  }
  gc_init(efp->gctp < 0 ? 0 : efp->gctp, efp->ifexp_result3, efp->ifexp_result4, efp->ifexp_result5);
  if (efp->argc > 1) {
    efp->tmp12 = strcmp((efp->argv)[1], "-s");
    efp->ifexp_result7 = efp->tmp12 == 0 ? 1 : 0;
  } else {
    efp->ifexp_result7 = 0;
  }
  ...
  if (efp->argc > 1) {
    efp->tmp13 = atoi((efp->argv)[1]);
    efp->ifexp_result8 = efp->tmp13;
  } else {
    efp->ifexp_result8 = 100000;
  }
  maxins = efp->ifexp_result8;
  if (efp->argc > 2) {
    efp->tmp14 = atoi((efp->argv)[2]);
    efp->ifexp_result9 = efp->tmp14;
  } else {
    efp->ifexp_result9 = 300000;
  }
  maxsearch = efp->ifexp_result9;
  printf("Bintree=%d, maxins=%d, maxsearch=%d\n", sizeof(Bintree), maxins, maxsearch);
  tmp_arg6 = &_Bintree_d;
  while ((efp->tmp15 = getmem(esp, &efp->scan1, tmp_arg6)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (int)0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  }
  efp->tmp_object4 = (struct _Bintree *) efp->tmp15;
  (*efp->tmp_object4).key = 0;
  (*efp->tmp_object4).val = 0;
  (*efp->tmp_object4).left = 0;
  (*efp->tmp_object4).right = 0;
  efp->root = efp->tmp_object4;
  while (randinsert(esp, &efp->scan1, efp->root, maxins), (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (int)0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  ...
  gettimeofday(&efp->tp1, 0);
  if (!efp->searchskip)
    while (bin2list(esp, &efp->scan1, efp->root, 0), (efp->tmp_esp = *((char **)esp)) != 0) {
      if ((char *)efp->tmp_esp < (char *)efp)
	return (int)0 - 1;
      efp->tmp_esp = 0;
      goto lgoto;
  ...
  gettimeofday(&efp->tp2, 0);
  efp->ttime = (efp->tp2).tv_sec - (efp->tp1).tv_sec + ((efp->tp2).tv_usec - (efp->tp1).tv_usec) * 1.0e-6;
  printf("total=%f gc=%f other=%f\n", efp->ttime, gc_ttime, efp->ttime - gc_ttime);
  return 0;
}
