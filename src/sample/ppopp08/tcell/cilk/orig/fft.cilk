...
static const char *ident __attribute__((__unused__))
     = "$HeadURL: https://bradley.csail.mit.edu/svn/repos/cilk/5.4.3/examples/fft.cilk $ $LastChangedBy: sukhaj $ $Rev: 517 $ $Date: 2003-10-27 10:05:37 -0500 (Mon, 27 Oct 2003) $";
 ...
 */
static cilk void compute_w_coefficients(int n, int a, int b, COMPLEX * W)
{
     ...
     if (b - a < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn compute_w_coefficients(n, a, ab, W);
	  spawn compute_w_coefficients(n, ab + 1, b, W);
     }
}
...
static cilk void unshuffle(int a, int b,
			   COMPLEX * in, COMPLEX * out, int r, int m)
{
     ...
     if (b - a < 16) {
	  ...
	  int ab = (a + b) / 2;
	  spawn unshuffle(a, ab, in, out, r, m);
	  spawn unshuffle(ab, b, in, out, r, m);
     }
}

static cilk void fft_twiddle_gen1(COMPLEX * in, COMPLEX * out,
...
static cilk void fft_twiddle_gen(int i, int i1,
				 COMPLEX * in, COMPLEX * out,
				 COMPLEX * W,
				 int nW, int nWdn, int r, int m)
{
     if (i == i1 - 1) {
	  spawn fft_twiddle_gen1(in + i, out + i, W,
				 r, m, nW, nWdn * i, nWdn * m);
     } else {
	  int i2 = (i + i1) / 2;
	  spawn fft_twiddle_gen(i, i2, in, out, W, nW,
				nWdn, r, m);
	  spawn fft_twiddle_gen(i2, i1, in, out, W, nW,
				nWdn, r, m);
     }
}
...
static cilk void fft_twiddle_2(int a, int b, COMPLEX * in, COMPLEX * out,
			       COMPLEX * W, int nW, int nWdn, int m)
{
     ...
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_twiddle_2(a, ab, in, out, W, nW, nWdn, m);
	  spawn fft_twiddle_2(ab, b, in, out, W, nW, nWdn, m);
     }
}
static cilk void fft_unshuffle_2(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     int i;
     const COMPLEX *ip;
     COMPLEX *jp;
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_unshuffle_2(a, ab, in, out, m);
	  spawn fft_unshuffle_2(ab, b, in, out, m);
     }
}
...
static cilk void fft_twiddle_4(int a, int b, COMPLEX * in, COMPLEX * out,
			       COMPLEX * W, int nW, int nWdn, int m)
{
     int l1, i;
     COMPLEX *jp, *kp;
     ...
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_twiddle_4(a, ab, in, out, W, nW, nWdn, m);
	  spawn fft_twiddle_4(ab, b, in, out, W, nW, nWdn, m);
     }
}
static cilk void fft_unshuffle_4(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     int i;
     const COMPLEX *ip;
     COMPLEX *jp;
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_unshuffle_4(a, ab, in, out, m);
	  spawn fft_unshuffle_4(ab, b, in, out, m);
     }
}
...
static cilk void fft_twiddle_8(int a, int b, COMPLEX * in, COMPLEX * out,
			       COMPLEX * W, int nW, int nWdn, int m)
{
     int l1, i;
     COMPLEX *jp, *kp;
     ...
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_twiddle_8(a, ab, in, out, W, nW, nWdn, m);
	  spawn fft_twiddle_8(ab, b, in, out, W, nW, nWdn, m);
     }
}
static cilk void fft_unshuffle_8(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     int i;
     const COMPLEX *ip;
     COMPLEX *jp;
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_unshuffle_8(a, ab, in, out, m);
	  spawn fft_unshuffle_8(ab, b, in, out, m);
     }
}
...
static cilk void fft_twiddle_16(int a, int b, COMPLEX * in, COMPLEX * out,
				COMPLEX * W, int nW, int nWdn, int m)
{
     int l1, i;
     COMPLEX *jp, *kp;
     ...
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_twiddle_16(a, ab, in, out, W, nW, nWdn, m);
	  spawn fft_twiddle_16(ab, b, in, out, W, nW, nWdn, m);
     }
}

static cilk void fft_unshuffle_16(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     int i;
     const COMPLEX *ip;
     COMPLEX *jp;
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_unshuffle_16(a, ab, in, out, m);
	  spawn fft_unshuffle_16(ab, b, in, out, m);
     }
}
...
static cilk void fft_twiddle_32(int a, int b, COMPLEX * in, COMPLEX * out,
				COMPLEX * W, int nW, int nWdn, int m)
{
     int l1, i;
     COMPLEX *jp, *kp;
     ...
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_twiddle_32(a, ab, in, out, W, nW, nWdn, m);
	  spawn fft_twiddle_32(ab, b, in, out, W, nW, nWdn, m);
     }
}

static cilk void fft_unshuffle_32(int a, int b, COMPLEX * in, COMPLEX * out, int m)
{
     int i;
     const COMPLEX *ip;
     COMPLEX *jp;
     if ((b - a) < 128) {
	  ...
	  int ab = (a + b) / 2;
	  spawn fft_unshuffle_32(a, ab, in, out, m);
	  spawn fft_unshuffle_32(ab, b, in, out, m);
     }
}
 ...
 *
 */
static cilk void fft_aux(int n, COMPLEX * in, COMPLEX * out, int *factors,
			 COMPLEX * W, int nW)
{
     int r, m;
     int k;
     ...
     if (r < n) {
	   ...
	   */
	  if (r == 32)
	       spawn fft_unshuffle_32(0, m, in, out, m);
	  else if (r == 16)
	       spawn fft_unshuffle_16(0, m, in, out, m);
	  else if (r == 8)
	       spawn fft_unshuffle_8(0, m, in, out, m);
	  else if (r == 4)
	       spawn fft_unshuffle_4(0, m, in, out, m);
	  else if (r == 2)
	       spawn fft_unshuffle_2(0, m, in, out, m);
	  else
	       spawn unshuffle(0, m, in, out, r, m);
	  sync;

	  for (k = 0; k < n; k += m) {
	       spawn fft_aux(m, out + k, in + k, factors + 1,
			     W, nW);
	  }
	  sync;
     }
      ...
      */
     if (r == 2)
	  spawn fft_twiddle_2(0, m, in, out, W, nW, nW / n, m);
     else if (r == 4)
	  spawn fft_twiddle_4(0, m, in, out, W, nW, nW / n, m);
     else if (r == 8)
	  spawn fft_twiddle_8(0, m, in, out, W, nW, nW / n, m);
     else if (r == 16)
	  spawn fft_twiddle_16(0, m, in, out, W, nW, nW / n, m);
     else if (r == 32)
	  spawn fft_twiddle_32(0, m, in, out, W, nW, nW / n, m);
     else
 ...
 */
cilk void cilk_fft(int n, COMPLEX * in, COMPLEX * out)
{
     ...
     spawn compute_w_coefficients(n, 0, n / 2, W);
     ...
     sync;			/* make sure W factors are computed */

     spawn fft_aux(n, in, out, factors, W, n);
     sync;
 ...
 */
cilk void test_fft_elem(int n, int j, COMPLEX * in, COMPLEX * out)
{
     int i;
     COMPLEX sum;
     COMPLEX w;
...
#define MAX 800
cilk void test_correctness(void)
{
     ...
     in1 = malloc(MAX * sizeof(COMPLEX));
     in2 = malloc(MAX * sizeof(COMPLEX));
     out1 = malloc(MAX * sizeof(COMPLEX));
     out2 = malloc(MAX * sizeof(COMPLEX));
     ...
     for (n = 1; n < MAX; ++n) {
	  ...
	  /* fft-ize */
	  spawn cilk_fft(n, in1, out1);
	  sync;
	  spawn test_fft(n, in2, out2);
	  sync;
...
cilk void test_speed(long size)
{
     COMPLEX *in, *out;
     int i;
     ...
     in = malloc(size * sizeof(COMPLEX));
     out = malloc(size * sizeof(COMPLEX));
     ...
     printf("Options:  number of elements   n = %ld\n\n", size);
...
int usage(void)
{
     ...
     fprintf(stderr, "This program is a highly optimized version of the classical\n");
     fprintf(stderr, "Cooley-Tukey Fast Fourier Transform algorithm.  Some documentation can\n");
     fprintf(stderr, "be found in the source code. The program is optimized for an exact\n");
...
{"-n", "-c", "-benchmark", "-h", 0};
int opt_types[] =
{LONGARG, BOOLARG, BENCHMARK, BOOLARG, 0};
