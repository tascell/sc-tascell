...
LGOTO:switch ((*efp).call_id) {
  ...
  *((char **)esp) = 0;
  ...
  {
    new_esp = esp;
    while (__builtin_expect(((tmp = search(new_esp, &efp->_bk, _thr, k, i0, i1, i2, maxp, maxk, &(*pthis))) == (int)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0),0)) {
      efp->maxk = maxk;
      efp->maxp = maxp;
      efp->i2 = i2;
      efp->i1 = i1;
...
LGOTO:switch ((*efp).call_id) {
  ...
  *((char **)esp) = 0;
  ...
  {
    new_esp = esp;
    while (__builtin_expect(((tmp2 = search(new_esp, &efp->_bk, _thr, 0, 0, 0, maxp, maxp, (efp->pen).maxk, &efp->pen)) == (int)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0),0)) {
      efp->kk = kk;
      efp->i = i;
      efp->maxp = maxp;
      efp->tmp2 = tmp2;
  ...
  *((char **)esp) = 0;
  s = 0;
  {
	  ...
	  }
	  {
		...
		if (kk == maxk) {
		  ...
		  if (ifexp_result9) {
		    ...
		    new_esp = esp;
		    while (__builtin_expect(((tmp3 = search(new_esp, &efp->dwind_bk, _thr, kk, j0 + 1, j0 + 1, maxp, maxp, maxk, tsk)) == (int)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0),0)) {
		      efp->tmp3 = tmp3;
    ...
    while (spawned-- > 0) {
      {
	new_esp = esp;
	while (__builtin_expect (((tmp4 = wait_rslt(new_esp, _thr)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0),0)) {
	  efp->tmp4 = tmp4;
	  efp->tmp3 = tmp3;
	  efp->ifexp_result9 = ifexp_result9;
	  efp->ifexp_result12 = ifexp_result12;
