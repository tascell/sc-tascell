...
;;;            ( a {b c} ) == ( a b c ) or ( a )
;;; 2004/07/29 ,symbol[...] の実装方式を変えた(reader が解釈するように)
;;;            []の中には関数名だけじゃなくてラムダ式も書けるようにした
...
(export '(sct mp-test *rule-path*))

(use-package "SC-MISC")

(defconstant *sctrans-package* *package*)
;;;;;
...
(defconstant *pattern-readtable* (make-pattern-readtable))
  ...
  (let ((out
    ...
    (when trace-p
      (format t "~A~S[(~S ~A)~%"
	      ...
	      (strcat (mapcar (compose #'prin1-to-string #'abbrev-list) out)
		      " ")))
...
(defun pattern-print (pat)
  (if (listp pat)
      (acond
       ((or (and (eq comma (car pat)) ",")
	    (and (eq comma-at (car pat)) ",@"))
	(format nil "~A~S~A"
		it
		(second pat)
		(aif (third pat)
       ...
       (t
	(format nil "(~A)"
		(strcat (mapcar #'pattern-print pat) " "))))
      (format nil "~S" pat)))
  ...
  (do* ((rpat pattern (cdr rpat))
    ...
    (cond
	 ...
	 ((and (eq comma (car cpat))
	       (symbolp (cadr cpat))) ; cpat = (comma <symbol> {<testfunc>}) 
	  (when (endp rx) (return nil))
	  (let ((sname (second cpat))
                (testfunc (third cpat))
		(retval nil))	       
	    (when (and testfunc
		       (not (car 
	 ...
	 ((and (eq comma-at (car cpat))
	       (symbolp (cadr cpat))) ; cpat = (comma-at <symbol> {<testfunc>})
...
(defmacro read-pattern (&rest read-args)
  `(in-readtable *pattern-readtable* (read ,@read-args)))

;;; 変形規則ファイルの読み込み
;;; 返り値は rule-list と flet-list(interpret実行用) の values
(defun load-rule (rule-file &optional (load-path *rule-path*)
		  &aux (rule-list nil) (flet-list nil) (input-data nil)
		  (*package* *sctrans-package*))
...
;;; sc-transformer(インタプリタ実行)
;;; sc-file-or-list: 変換対象のSCファイル名またはS式
;;; rule-file: 変換規則ファイル名 またはそのリスト
;;; output-file: 出力ファイル
;;; mac: 事前にマクロ展開を行うか(t or nil)
(defun sct (sc-file-or-list rule-file &key output-file (mac t)
	      &aux (*rule-list* nil) (*flet-list* nil))
  ;; rule-fileが複数指定された場合
  (when (listp rule-file)
    (when (null rule-file) (return-from sct sc-file-or-list))
    (format t "applying ~S:~%" (car rule-file))
    (return-from sct
       (sct
	(sct sc-file-or-list (car rule-file) :mac mac
