...
(defun get-frame (x)
  (let* ((strx (par-identifier x))
    ...
    (if asc
	(cdr asc)
      (let* ((basename (string+ (list strx "_frame")))
  ...
  (mapcan
   #'(lambda (x)
       (cond ((eq 'closure-t (cdr x))
	     ...
	     ((and (listp (cdr x))
		   (eq 'array (cadr x)))
	      (let ((atype (second (cdr x)))
		    (alength (third (cdr x))))
		`( (memcpy (fref efp -> ,(car x)) ,(car x)
		    (* (sizeof ,atype) ,alength)) )
		))
	     (t
