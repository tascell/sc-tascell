...
static char *
med3 (char *a, char *b, char *c, CTOLD int (*cmp) CT (void const *, void const *))
{
  return cmp (a,
	      b) < 0 ? (cmp (b,
...
quicksort (char *a, size_t n, size_t es,
	   CTOLD int (*cmp) CT (void const *, void const *))
{
  char *pa;
  char *pb;
  char *pc;
...
modsort (int *a, int n, int m)
{
  ...
  CTOLD int comp_mod CT (int *pp, int *pq)
  {
    return *pp % m < *pq % m ? 1 : (*pp % m == *pq % m ? 0 : -1);
  }
