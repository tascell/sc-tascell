...
int
nqueens (int (*_bk) (void), struct thread_data *_thr, int n, int k, int ix,
         int iy, struct nqueens *tsk)
{
  int s = 0;
  {
    int i = ix;
...
void
do_nqueens_task (struct thread_data *_thr, struct nqueens *pthis)
{
  ...
  if (k < 0)
    ...
    {
      (*pthis).r =
        nqueens (_bk, _thr, n, k, (*pthis).i1, (*pthis).i2, &(*pthis));
    }
