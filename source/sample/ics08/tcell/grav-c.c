...
void grav_x (int minx, int miny, int minz, int maxx, int maxy, int maxz,
             struct gravity *tsk);
...
/*  fprintf (stderr, "start (%d, %d, %d)--(%d, %d, %d): (%lf, %lf, %lf)\n",
           ...
           ((*pthis).cur)[2]);
*/  grav_x ((*pthis).minx, (*pthis).miny, (*pthis).minz, (*pthis).maxx,
...
do_start_gravity_task (struct thread_data *_thr, struct start_gravity *pthis)
{
  ...
  grav_x (tsk.minx, tsk.miny, tsk.minz, tsk.maxx, tsk.maxy, tsk.maxz, &tsk);
  {
    i = 0;
    for (; i < 3; i++)
      {
...
void
grav_z (int x, int y, int minz, int maxz, struct gravity *tsk)
{
  int i;
  {
    int z = minz;
...
void
grav_y (int x, int miny, int minz, int maxy, int maxz, struct gravity *tsk)
{
  {
    int y = miny;
    int y_end = maxy + 1;
    for (; y < y_end; y++)
      {
        grav_z (x, y, minz, maxz, tsk);
      }
  }
}

void
grav_x (int minx, int miny, int minz, int maxx, int maxy, int maxz,
        struct gravity *tsk)
{
  {
    int x = minx;
    ...
    for (; x < x_end; x++)
      {
        grav_y (x, miny, minz, maxy, maxz, tsk);
      }
  }
}
