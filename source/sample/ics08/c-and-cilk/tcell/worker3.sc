...
(%if (eq 'FN 'sc::lightweight)
    ((%error "lightweight")
     (%rule (:nestfunc-sc1 :nestfunc-type :nestfunc-temp :nestfunc :untype))
     (%output-file "worker2-lw.c")))

(%ifdef clsc
    ((%error "closure")
     (%rule (:nestfunc-sc1 :nestfunc-type :nestfunc-temp
                           (:nestfunc :all-in-estack t) :untype))
     (%output-file "worker2-clos.c")))

(static-def debug int 1)
...
(def cmd-v-buf (array char 512 MAXCMDC 256))
(def cmd-in (ptr (struct cmd-list)))
(def cmd-out (ptr (struct cmd-list)))
(def buf (array char BUFSIZE))
(def divisibility-flag int 0)           ; 今度do-two等に来たら"dvbl"を送ろうフラグ

;;; fgetsでOUTSIDEからのメッセージを受け取る -> struct cmd
(def (csym::recv-command) (csym::fn (struct cmd))
  (def r (struct cmd))
  (defs char p c)
  (def b (ptr char) buf)
...
(def (csym::send-command cmd) (csym::fn void (struct cmd))
  (def i int)
...
;;; エラーメッセージstrをstderrへ
(def (csym::proto-error str cmd) (csym::fn void (ptr (const char)) (struct cmd))
  (def i int)
  (csym::fputs str stderr)
  (csym::fputc #\Newline stderr)
  (for ((= i 0) (< i (- (fref cmd c) 1)) (inc i))
...
(def (csym::enqueue-command cmd body)
    (csym::fn void (struct cmd) (ptr void))
  (def i int)
  (def len size-t)
...
(def snr-in (ptr (struct cmd-list)))    ; none, rackだけたまるqueue
(def snr-out (ptr (struct cmd-list)))

(def (csym::enqueue-snr cmd) (csym::fn void (struct cmd))
  (def i int)
  (def len size-t)
  (def q (ptr (struct cmd-list)))
...
(def (csym::flush-treq-with-none-1 thr) (csym::fn void (ptr (struct thread-data)))
  (def rcmd (struct cmd))
...
(def (csym::flush-treq-with-none thr) (csym::fn void (ptr (struct thread-data)))
  ;; treq がたまっていたらそれらを破棄したうえで noneを送る
  ;; thr->mut は lock済
...
(def (recv-exec-send thr treq-head req-to)
  ...
  ((aref task-doers (fref tx -> task-no)) thr (fref tx -> body)) ; タスク実行（tcell変更） 
  (csym::pthread-mutex-lock (ptr (fref thr -> mut)))
  ...
  (if (== (fref rcmd node) INSIDE)
    ...
    (if (== (fref rcmd node) OUTSIDE)
        (begin                          ; 外部に結果送信
         ...
         ((aref rslt-senders (fref tx -> task-no)) thr (fref tx -> body)) ;tcell変更
...
(def (csym::recv-task cmd body) (csym::fn void (struct cmd) (ptr void))
  (def tx (ptr (struct task)))
  ...
  ;; スレッドに情報を与える．
  (csym::pthread-mutex-lock (ptr (fref thr -> mut)))
  (= tx (fref thr -> task-top))         ; このtxにやってほしいtask情報を書き込む
  (= (fref tx -> rslt-to) (fref cmd node)) ; 結果の送り先種別
  ...
  (= (fref tx -> task-no) (csym::atoi (aref (fref cmd v) 4))) ; タスク番号（tcell追加）
  ...
  (if (== (fref cmd node) INSIDE)
    ...
    (if (== (fref cmd node) OUTSIDE)
        (begin                          ; OUTSIDEだったら文字列で読み込む
         (= (fref tx -> body)           ; 読み出しはロック獲得の前のほうがいいかも
            ((aref task-receivers (fref tx -> task-no)) thr)) ; tcell変更
         (csym::read-to-eol))
      (begin
  ...
  (csym::pthread-mutex-unlock (ptr (fref thr -> mut))) 
  ;; ノード内に待っているスレッド(ワーカ)を起こす
  (csym::pthread-cond-broadcast (ptr (fref thr -> cond)))
  )

(def (csym::recv-none cmd) (csym::fn void (struct cmd))
  (def thr (ptr (struct thread-data)))
  (def id unsigned-int)
  (def len size-t)
  (if (< (fref cmd c) 2)
...
(def (csym::recv-rslt cmd rbody) (csym::fn void (struct cmd) (ptr void))
  (def rcmd (struct cmd))
  (def thr (ptr (struct thread-data)))
  (def hx (ptr (struct task-home)))
  ...
  (if (== (fref cmd node) INSIDE)
    ...
    (if (== (fref cmd node) OUTSIDE)    ; OUTSIDEならストリームから
        (begin
         ...
         ((aref rslt-receivers (fref hx -> task-no)) thr (fref hx -> body)) ;tcell変更
         (csym::read-to-eol))
      (begin
...
(def (csym::try-treq cmd id) (csym::fn int (struct cmd) unsigned-int)
  (def hx (ptr (struct task-home)))
  (def thr (ptr (struct thread-data)))
  (def len size-t)
  (def avail int 0)
...
(def (csym::recv-treq cmd) (csym::fn void (struct cmd))
  ...
  (def rcmd (struct cmd))
  (def id unsigned-int)
  (if (< (fref cmd c) 3)                ; 引数の数チェック 0:"treq", 1:from, 2:to
  ...
  (if (== (csym::strcmp (aref (fref cmd v) 2) "any") 0)
    ...
    (begin                              ; "any"じゃない場合
     ...
     (if (not (< id num-thrs))
         (csym::proto-error "Wrong task-head" cmd))
     (if (csym::try-treq cmd id)        ; treqできた
         (return))))
...
(def (csym::recv-rack cmd) (csym::fn void (struct cmd))
  (def tx (ptr (struct task)))
  (def thr (ptr (struct thread-data)))
  (def id unsigned-int)
  (def len size-t)
...
(def (csym::make-and-send-task thr task-no body) ; task-noをtcell追加
  ...
  (= (aref (fref tcmd v) 2) (fref thr -> buf)) ; 結果返答先（自分）
  ...
  (if (== (fref tcmd node) INSIDE)
    ...
    (if (== (fref tcmd node) OUTSIDE)
        (begin                          ; 外部へ送信
         ...
         ((aref task-senders task-no) thr body) ; tcell変更
         (csym::write-eol)
         (csym::flush-send)
         (csym::pthread-mutex-unlock (ptr send-mut))
         (csym::pthread-mutex-lock (ptr (fref thr -> mut))))
...
(def (main argc argv) (fn int int (ptr (ptr char)))
  ...
  (systhr-create exec-queue-cmd NULL)   ; メッセージ受信・処理スレッド
  ...
  (systhr-create send-none-rack NULL)   ; none, rack送信スレッド
  ...
  (while (exps (= cmd (csym::recv-command))
               (and (> (fref cmd c) 0)
                    (!= 0 (csym::strcmp
