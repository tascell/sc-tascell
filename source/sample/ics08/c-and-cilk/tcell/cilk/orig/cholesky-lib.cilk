...
static const char *ident __attribute__((__unused__))
     = "$HeadURL: https://bradley.csail.mit.edu/svn/repos/cilk/5.4.3/examples/cholesky-lib.cilk $ $LastChangedBy: bradley $ $Rev: 517 $ $Date: 2004-06-22 00:03:24 -0400 (Tue, 22 Jun 2004) $";
...
char *specifiers[] =
{"--nproc", "-n", "-z", "-f", "-benchmark", "-h", 0};
int opt_types[] =
{INTARG, INTARG, INTARG, STRINGARG, BENCHMARK, BOOLARG, 0};

Matrix EXPORT(copy_matrix)(CilkContext *const context, int depth, Matrix a);
Matrix EXPORT(cholesky)(CilkContext *const context, int depth, Matrix a);
Matrix EXPORT(mul_and_subT)(CilkContext *const context, int depth, int lower, Matrix a, Matrix b, Matrix r);

int main(int argc, char *argv[])
{
     Matrix A, R;
     int cilk_active_size, size, depth, nonzeros, i, benchmark, help;
     ...
     get_options(argc, argv, specifiers, opt_types,
		 &cilk_active_size, &size, &nonzeros, filename, &benchmark, &help);
     ...
     R =  EXPORT(copy_matrix)(context, depth, A);

     input_blocks = num_blocks(depth, R);
     input_nonzeros = num_nonzeros(depth, R);

     R = EXPORT(cholesky)(context, depth, R);
     ...
     A = EXPORT(mul_and_subT)(context, depth, 1, R, R, A);
