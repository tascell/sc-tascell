...
void
send_cmp_task (struct thread_data *_thr, struct cmp *pthis)
{
  send_int ((*pthis).n1);
  send_int ((*pthis).n2);
  int i;
...
struct cmp *
recv_cmp_task (struct thread_data *_thr)
{
...
void
send_cmp_rslt (struct thread_data *_thr, struct cmp *pthis)
{
  send_int ((*pthis).r);
  free ((*pthis).d1);
  free ((*pthis).d2);
...
void
recv_cmp_rslt (struct thread_data *_thr, struct cmp *pthis)
{
  (*pthis).r = recv_int ();
}
