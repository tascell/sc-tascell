...
int *show_boards[8]={0,0,0,0,0,0,0,0};
gushort bg_colors[8]={0x1111,0x1111,0x1111,0x1111,0x1111,0x1111,0x1111,0x1111};
extern GdkPixmap *pixmap;

gushort rcols[12]={0x0000, 0x8888, 0xffff, 0x0000, 0x8888, 0xffff, 0x0000, 0x8888, 0xffff, 0x0000, 0x8888, 0xffff};
gushort gcols[12]={0x8888, 0x0000, 0x0000, 0x8888, 0x8888, 0x8888, 0xffff, 0xffff, 0xffff, 0x0000, 0x0000, 0x0000};
gushort bcols[12]={0x8888, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x8888, 0x8888, 0x8888};
...
gint repaint (gpointer data)
{
    ...
    for (i=0 ; i<8 ; i++) {
        if (show_boards[i]==0) break;
        for (x=0; x<6; x++)
            for (y=0; y<10; y++) {
                int ps = *(show_boards[i]+x+7*y);
                gushort r, g, b;
                if (ps==0) { r=g=b=bg_colors[i]; }
                else {
                    ps=ps-'A';
                    if (ps<0 || ps>12) fprintf ("ps=%d\n", ps);
                    r=rcols[ps]; g=gcols[ps]; b=bcols[ps];
                }
                gdk_draw_rectangle (pixmap, set_color (r,g,b), TRUE,
                                    x*bsize+i*8*bsize, y*bsize, bsize-1, bsize-1);
            }
    }
...
int search (int (*_bk) (void), struct thread_data *_thr, int k, int j0,
...
do_pentomino_task (struct thread_data *_thr, struct pentomino *pthis)
{
    ...
    show_boards[_thr->id]=pthis->b;
    (*pthis).r = search (_bk, _thr, k, i0, i1, i2, &(*pthis));
    show_boards[_thr->id]=0;
                         ...
                         struct start_pentomino *pthis)
{
    ...
    gettimeofday (tp, 0);
    show_boards[_thr->id]=pen.b;
    (*pthis).r = search (_bk, _thr, 0, 0, 0, 12, &pen);
    show_boards[_thr->id]=0;
    gettimeofday (tp + 1, 0);
...
int
search (int (*_bk) (void), struct thread_data *_thr, int k, int j0, int j1,
        int j2, struct pentomino *tsk)
{
    int s = 0;
    {
        int p = j1;
        ...
        if ((*_thr).req)
            {
                bg_colors[_thr->id]=0xffff;
                handle_req (do_many_bk, _thr);
                bg_colors[_thr->id]=0x1111;
            }
        for (; p < p_end; p++)
            {
                int ap = ((*tsk).a)[p];
                                ...
                                }
                                {
                                    ...
                                    }
                                    {
                                        ...
                                        if (kk == 70)
                                            ...
                                            {
                                                s +=
                                                    search (dwind_bk, _thr, kk, j0 + 1, j0 + 1, 12,
                                                            tsk);
                                            }
                                    }
                                    {
