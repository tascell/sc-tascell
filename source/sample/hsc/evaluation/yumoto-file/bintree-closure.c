...
lgoto:
  ...
  *((char **)esp) = 0;
  ...
  efp->searchskip = 0;
  if (efp->argc > 3) {
    efp->tmp6 = atoi((efp->argv)[3]);
    efp->ifexp_result = efp->tmp6;
  } else {
    efp->ifexp_result = 0;
  }
  efp->gctp = efp->ifexp_result;
  if (efp->argc > 4) {
    efp->tmp7 = atoi((efp->argv)[4]);
    efp->ifexp_result3 = efp->tmp7;
  } else {
    efp->ifexp_result3 = 0;
  }
  if (efp->argc > 5) {
    efp->tmp8 = atoi((efp->argv)[5]);
    efp->ifexp_result4 = efp->tmp8;
  } else {
    efp->ifexp_result4 = 0;
  }
  if (efp->argc > 6) {
    efp->tmp9 = atoi((efp->argv)[6]);
  ...
  if (efp->argc > 1) {
    efp->tmp10 = strcmp((efp->argv)[1], "-s");
    efp->ifexp_result7 = efp->tmp10 == 0 ? 1 : 0;
  } else {
    efp->ifexp_result7 = 0;
  }
  ...
  tmp_arg2 = &_Bintree_d;
  while ((efp->tmp13 = getmem(esp, &efp->scan1, tmp_arg2)) == (void *)0 - 1 && (efp->tmp_esp = *((char **)esp)) != 0) {
    if ((char *)efp->tmp_esp < (char *)efp)
      return (int)0 - 1;
    efp->tmp_esp = 0;
    goto lgoto;
  }
  efp->tmp_object2 = (struct _Bintree *) efp->tmp13;
  (*efp->tmp_object2).key = 0;
  (*efp->tmp_object2).val = 0;
  (*efp->tmp_object2).left = 0;
  (*efp->tmp_object2).right = 0;
  efp->root = efp->tmp_object2;
  gettimeofday(&efp->tp1, 0);
  ...
  gettimeofday(&efp->tp2, 0);
  if (!efp->searchskip)
    while (randsearch(esp, &efp->scan1, efp->root, maxsearch), (efp->tmp_esp = *((char **)esp)) != 0) {
      if ((char *)efp->tmp_esp < (char *)efp)
	return (int)0 - 1;
      efp->tmp_esp = 0;
      goto lgoto;
