...
typedef lightweight void    (*sht) (void);
...
Alist          *
bin2list(sht scan0, Bintree * x, Alist * rest)
{
...
lightweight  void            scan1(void){
  ...
  if              ((*x).right) {
    rest = bin2list(scan1, (*x).right, rest);
    (*x).right = 0;
  }
  kv = (((tmp_object2 = (struct _KVpair *) getmem(scan1, &_KVpair_d), (*tmp_object2).key = (*x).key), (*tmp_object2).val = (*x).val), tmp_object2);
  a = (((tmp_object3 = (struct _Alist *) getmem(scan1, &_Alist_d), (*tmp_object3).kv = kv), (*tmp_object3).cdr = rest), tmp_object3);
  rest = a;
  if ((*x).left) {
    rest = bin2list(scan1, (*x).left, rest);
    (*x).left = 0;
  }
  return rest;
}
...
lightweight  void            scan1(void){
  ...
  root = (((((tmp_object4 = (struct _Bintree *) getmem(scan1, &_Bintree_d), (*tmp_object4).key = 0), (*tmp_object4).val = 0), (*tmp_object4).left = 0), (*tmp_object4).right = 0), tmp_object4);
