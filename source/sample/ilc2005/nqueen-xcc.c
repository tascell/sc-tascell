...
int nqueens(proc_env pr, lightweight int (*bkf)(req_buf_t *), int n, int k);

void tf_backup_and_nqueens(proc_env pr, task_buf_t *task_buf_p){
  lightweight int bk(req_buf_t *req_buf_p){ return 0; }
  int a_bak[20];
  int lb_bak[40];
  int rb_bak[40];
  int jj;
...
int 
nqueens(proc_env pr, lightweight int (*bkf)(req_buf_t *), int n, int k)
{
  if (n == k)
    return 1;
  else
    {
      ...
      lightweight int bk(req_buf_t *req_buf_p){
      ...
      while(si < ei)
	{
	  ...
	  POLL(pr->req_port, bk);
	  {
	    ...
	    if (! (pr->lb[n - 1 - ai + k] || pr->rb[ai + k])){
	      ...
	      pr->lb[n - 1 - ai + k] = 1; pr->rb[ai + k] = 1;
	      {
		lightweight int bk2(req_buf_t *req_buf_p){
		  int r;
		  ai = pr->a[k]; ak = pr->a[i];
		  pr->a[i] = ai; pr->a[k] = ak;
		  pr->lb[n - 1 - ai + k] = 0; pr->rb[ai + k] = 0;
