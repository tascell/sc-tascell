...
\noindent
declaration: \\
\quad {\tt (}storage-class-specifier identifier type-expression initializer\opt {\tt )} \\
\quad {\tt (}storage-class-specifier {\tt (}identifier-list{\tt )} {\tt (fn} function-type-list{\tt )} {\rm [}{\tt :attr} function-attribute{\rm ]}\opt register-declarator\opt block-item-list\opt {\tt )} \\ 
...
\quad {\it def-or-decl} \\
\quad extern extern-def extern-decl static static-def \\
...
initializer-list: \\
\quad initializer \\
\quad initializer-list initializer \\
...
\quad = *= /= \%= += -= <<= >>= bit-and= but-xor= bit-or= \\ \it
